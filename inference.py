...
def moses_benchmark(model, opt):
    ...
    print("\nGenerating molecules for MOSES benchmarking...", flush=True)
    n_samples = 30000
    nBins = [100] * opt.cond_dim

    data = opt.data[opt.cond_labels]#pd.read_csv(opt.data)[opt.cond_labels]
    toklen_data = pd.read_csv(opt.load_toklendata)

    conds = rand_gen_from_data_distribution(data, size=n_samples, nBins=nBins, cond_dim=opt.cond_dim)
        ...
        while True:
            try:
                ...
                if opt.tokenizer == "smilespe":
                    ...
                    molecule_tmp = atomInSmiles.decode(molecule_tmp)

                molecules.append(molecule_tmp)
                conds_trg.append(conds[idx])
                toklen_check.append(toklen-opt.cond_dim)# toklen - cond_dim: due to cond dim
                
                m = Chem.MolFromSmiles(molecule_tmp)
                val_check.append(0 if m is None else 1)
                break
            except:
                continue
    ...
    gen_list = pd.DataFrame({
        "SMILES": molecules,
        "Valid": val_check,
        "Temperature": np_conds_trg[:, 0],
        "DynViscosity": np_conds_trg[:, 1],
        "Density": np_conds_trg[:, 2],
        "ElecConductivity": np_conds_trg[:, 3],
        "toklen": toklen_check, 
        "toklen_gen": toklen_gen
    })
    # gen_list = pd.read_csv("IL_moses_bench2_lat=128_epo=0_k=4_20230710.csv")
    gen_list[opt.cond_labels] = opt.robust_scaler.inverse_transform(gen_list[opt.cond_labels])
    # gen_list.to_csv('IL_moses_bench2_lat=128_epo=0_k=4_20230710.csv', index=False)

    gen_list.to_csv('{}_IL_moses_bench2_lat={}_epo={}_k={}_{}.csv'.format(opt.tokenizer, opt.latent_dim, opt.epochs, opt.k, time.strftime("%Y%m%d")), index=False)

    print("Please check the file: '{}_IL_moses_bench2_lat={}_epo={}_k={}_{}.csv'".format(opt.tokenizer, opt.latent_dim, opt.epochs, opt.k, time.strftime("%Y%m%d")))


def ten_condition_test(model, opt):
    idxs, molecules, val_check, conds_trg, toklen_check, toklen_gen = [], [], [], [], [], []
    ...
    print("conds:\n", opt.robust_scaler.inverse_transform(conds))
    ...
    gen_list.to_csv('{}_moses_bench2_10conds_lat={}_epo={}_k={}_{}.csv'.format(opt.tokenizer, opt.latent_dim, opt.epochs, opt.k, time.strftime("%Y%m%d")), index=True)

    print("Please check the file: '{}_moses_bench2_10conds_lat={}_epo={}_k={}_{}.csv'".format(opt.tokenizer, opt.latent_dim, opt.epochs, opt.k, time.strftime("%Y%m%d")))
...
def get_program_arguments():
    ...
    parser.add_argument('-load_weights', type=str, default="weights/smilespe_lr=1e-05_drop=0.2_e=50_b=256_nl=6_h=8_d=256_lat=128")
    parser.add_argument('-load_data', type=str, default='data/IL/train.csv')
